...
const assessmentsController = {
  addAssessment: async (req, res) => {
      const { title, description, assessment_type, questions, solution, pass_percentage, module_id } = req.body;

      if (!title || !assessment_type || !questions || !solution || !pass_percentage || !module_id) {
      ...
      try {
          const result = await pool.query(
              ...
              [title, description, assessment_type, questions, solution, pass_percentage, module_id]
          );

          res.status(201).json({ message: "Assessment created", assessment: result.rows[0] });
      } catch (err) {
  ...
  updateAssessment: async (req, res) => {
      const { id } = req.params;
      const { title, description, assessment_type, questions, solution, pass_percentage } = req.body;
      ...
      try {
          const result = await pool.query(
              ...
              [title, description, assessment_type, questions, solution, pass_percentage, id]
          );
